...
def analyze_oct(image, pdf_path, prompt_text):
    ...
    # Use Gemini 1.5 Flash (Most stable free model)
...
with col2:
    ...
    if st.button("Analyze Scan", type="primary"):
        if not api_key:
            ...
            # Run Analysis (Direct call, no try/except block to avoid indentation errors)
            with st.spinner("Analyzing layers and consulting the textbook..."):
                result = analyze_oct(image, tmp_pdf_path, system_prompt)
                st.markdown(result)
            
            # Cleanup temp file
            os.remove(tmp_pdf_path)import streamlit as st
import google.generativeai as genai
from PIL import Image
import tempfile
import os
